"""
Current:
Changing the model to a pretrained ResNet50

Last:
Changing the model to U-Net
...
ODIR = '../output/fcn'
STUDY = '' if len(sys.argv) < 2 else sys.argv[1]
STUDY = STUDY if STUDY != '' else f'{time.time():.0f}'
NTRIAL = 20
NEPOCH = 100
NBATCH = 5
NROWS, NCOLS = 512, 512
...
class Train:
    ...
    def _train(self, model, crit, optim):
        ...
        for bc in range(1, self.data.shape[0] // self.bsize + 1 +
            ...
            optim.zero_grad()
            out = model(batch)['out'].squeeze(dim=1).sigmoid()
...
if __name__ == "__main__":
    model_dir = 'model_' + STUDY
    model_dir = os.path.join(ODIR, model_dir)
