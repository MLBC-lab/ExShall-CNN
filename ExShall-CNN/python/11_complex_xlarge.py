...
#     """
#     Conv2d implementation with complex weight and bias
#     """
#     def __init__(self, in_channels, out_channels,
#                  kernel_size, stride, dilation=1, padding=0,
#                  bias=True):
...
#                                       bias=bias)
#         self.conv2d.weight = torch.nn.Parameter(
#             torch.complex(torch.randn(self.conv2d.weight.shape),
#                           torch.randn(self.conv2d.weight.shape)))
#         self.conv2d.bias = torch.nn.Parameter(
#             torch.complex(torch.randn(self.conv2d.bias.shape),
#                           torch.randn(self.conv2d.bias.shape)))
#     def forward(self, x):
    ...
    def __init__(self, neighbor_radius=[3], dilation=[1],
        ...
        for nei, dil in zip(neighbor, dilation):
            ...
            layer.append(torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=3, out_channels=ochannel,
            ...
            layer.append(torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=3, out_channels=ochannel,
                                ...
                                dilation=dil, padding='same', bias=True,
                                dtype=torch.complex64),
                torch.nn.Conv2d(in_channels=ochannel, out_channels=ochannel,
                                kernel_size=nei, stride=1,
                                dilation=dil, padding='same', bias=True,
                                dtype=torch.complex64)))
            # Exp(Log(summation))
            ...
            layer.append(torch.nn.Sequential(
                ...
                torch.nn.Conv2d(in_channels=ochannel, out_channels=ochannel,
                                ...
                                dtype=torch.complex64),
                Exp()))
            # Exp(Log(Exp(Log(Exp(Log(summation))))))
            layer.append(torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=3, out_channels=ochannel,
                                kernel_size=nei, stride=1,
                                dilation=dil, padding='same', bias=True,
        ...
        self.aggregate = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=comb.shape[1], out_channels=ochannel,
