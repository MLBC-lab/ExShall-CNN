        ...
        for nei, dil in zip(neighbor, dilation):
            ...
            layer.append(torch.nn.Sequential(
                ...
                torch.nn.Conv2d(in_channels=ochannel,
                                ...
                                bias=True),
                Exp()))
            layer.ops[-1][2].weight = torch.nn.Parameter(
                torch.complex(
                    torch.randn(layer.ops[-1][2].weight.shape),
                    torch.randn(layer.ops[-1][2].weight.shape)))
            layer.ops[-1][2].bias = torch.nn.Parameter(
                torch.complex(
                    torch.randn(layer.ops[-1][2].bias.shape),
                    torch.randn(layer.ops[-1][2].bias.shape)))
